import { Stack } from 'expo-router';
import LegalDocument from '../../components/LegalDocument';

const EFFECTIVE_DATE = '2026년 3월 14일';

const SECTIONS = [
  {
    title: '제1조 (목적)',
    content: '본 약관은 "위시맵"(이하 "앱")이 제공하는 모든 서비스의 이용 조건 및 절차, 이용자와 앱 운영자의 권리·의무 및 책임 사항을 규정함을 목적으로 합니다.',
  },
  {
    title: '제2조 (정의)',
    content: `• "서비스"란 앱을 통해 제공되는 맛집 지도, 맛집 제안, 북마크, 댓글 등 관련 기능을 말합니다.
• "이용자"란 본 약관에 따라 앱이 제공하는 서비스를 이용하는 자를 말합니다.
• "계정"이란 이용자의 식별 및 서비스 이용을 위해 소셜 로그인으로 생성된 회원 정보를 말합니다.`,
  },
  {
    title: '제3조 (약관의 효력 및 변경)',
    content: `① 본 약관은 앱 내에 게시하거나 기타 적절한 방법으로 이용자에게 공지함으로써 효력을 발생합니다.
② 앱 운영자는 관련 법령을 위배하지 않는 범위에서 본 약관을 변경할 수 있으며, 변경 시 적용일 7일 전부터 앱 내 공지를 통해 안내합니다.
③ 변경된 약관에 동의하지 않는 이용자는 서비스 이용을 중단하고 회원 탈퇴를 할 수 있습니다.`,
  },
  {
    title: '제4조 (서비스의 내용)',
    content: `앱은 다음의 서비스를 제공합니다.

• 지도 기반 맛집 탐색 및 조회
• 맛집 제안 (사용자가 새로운 맛집을 제안)
• 좋아요 및 북마크 기능
• 댓글 작성 및 조회
• 기타 앱 운영자가 추가 개발하여 제공하는 서비스`,
  },
  {
    title: '제5조 (회원가입 및 계정)',
    content: `① 이용자는 카카오, 구글, 네이버, Apple 소셜 로그인을 통해 회원가입을 할 수 있습니다.
② 이용자는 정확한 정보를 제공해야 하며, 허위 정보 입력으로 인한 불이익은 이용자에게 귀속됩니다.
③ 로그인 없이도 지도 조회 등 일부 서비스를 이용할 수 있습니다.`,
  },
  {
    title: '제6조 (서비스 이용)',
    content: `① 서비스는 연중무휴 24시간 제공을 원칙으로 합니다. 다만, 시스템 점검 등의 사유로 서비스가 일시 중단될 수 있습니다.
② 앱에서 제공하는 맛집 정보는 이용자가 제안한 콘텐츠이며, 그 정확성을 보장하지 않습니다.`,
  },
  {
    title: '제7조 (이용자의 의무)',
    content: `이용자는 다음 행위를 해서는 안 됩니다.

• 타인의 개인정보를 도용하는 행위
• 허위 맛집 정보를 등록하는 행위
• 서비스의 운영을 방해하는 행위
• 욕설, 비방, 혐오 표현 등 부적절한 콘텐츠를 작성하는 행위
• 스팸, 광고성 콘텐츠를 게시하는 행위
• 음란물 또는 폭력적인 콘텐츠를 게시하는 행위
• 타인을 괴롭히거나 위협하는 행위
• 기타 관련 법령에 위반되는 행위`,
  },
  {
    title: '제8조 (콘텐츠 관리 및 신고)',
    content: `① 앱 운영자는 부적절한 콘텐츠에 대해 무관용 원칙(zero tolerance)을 적용합니다.
② 이용자는 부적절한 콘텐츠를 발견한 경우 앱 내 신고 기능을 통해 신고할 수 있습니다.
③ 신고된 콘텐츠는 24시간 이내에 검토되며, 위반이 확인된 경우 즉시 삭제됩니다.
④ 부적절한 콘텐츠를 게시한 이용자는 사전 경고 없이 서비스 이용이 제한될 수 있습니다.
⑤ 이용자는 다른 이용자를 차단하여 해당 이용자의 콘텐츠가 표시되지 않도록 할 수 있습니다.`,
  },
  {
    title: '제9조 (서비스의 변경 및 중단)',
    content: `① 앱 운영자는 운영상 또는 기술상의 필요에 따라 서비스를 변경할 수 있습니다.
② 천재지변, 시스템 장애 등 불가피한 사유로 서비스 제공이 어려운 경우 일시적으로 서비스를 중단할 수 있습니다.`,
  },
  {
    title: '제10조 (회원 탈퇴 및 자격 상실)',
    content: `① 이용자는 앱 내 설정에서 언제든지 회원 탈퇴를 요청할 수 있으며, 앱 운영자는 즉시 처리합니다.
② 회원 탈퇴 시 이용자의 모든 데이터는 즉시 삭제되며 복구할 수 없습니다.`,
  },
  {
    title: '제11조 (면책 조항)',
    content: `① 이용자가 제안한 맛집 정보의 정확성에 대해 앱 운영자는 책임을 지지 않습니다.
② 천재지변 또는 이에 준하는 불가항력으로 인해 서비스를 제공할 수 없는 경우 책임이 면제됩니다.`,
  },
  {
    title: '제12조 (분쟁 해결)',
    content: `① 앱 운영자와 이용자 간 발생한 분쟁에 대해 양측은 원만한 해결을 위해 성실히 협의합니다.
② 협의가 이루어지지 않는 경우 관련 법령에 따른 절차를 따릅니다.`,
  },
  {
    title: '부칙',
    content: `본 약관은 ${EFFECTIVE_DATE}부터 시행합니다.`,
  },
];

export default function TermsScreen() {
  return (
    <>
      <Stack.Screen options={{ title: '이용약관' }} />
      <LegalDocument title="이용약관" effectiveDate={EFFECTIVE_DATE} sections={SECTIONS} />
    </>
  );
}
